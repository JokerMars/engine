	...
	try {
		...
		{//if reference decreases to 0, write file flag, flush|purge cache, and delete file context
				...
				)
			{
				...
				RtlCopyMemory(wszFileDosFullPath + 4 + pVolCtx->Name.Length / sizeof(WCHAR), pszRelativePathPtr,
					pStreamCtx->FileName.Length - wcslen(pStreamCtx->wszVolumeName) * sizeof(WCHAR));
